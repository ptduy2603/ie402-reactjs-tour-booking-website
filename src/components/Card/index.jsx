...
const TourCard = ({ tour }) => {
  return (
    <Link to={`/detail/${tour.id}`} className="border rounded-lg shadow hover:shadow-xl transition duration-300 overflow-hidden transform hover:scale-100 hover:-translate-y-2 cursor-pointer">
      <img
        src={tour.image}
        alt={tour.title}
        ...
        <div className="flex items-center text-gray-600 text-base">
          {[...Array(5)].map((_, index) => (
            <FontAwesomeIcon
              icon={faStar}
              key={index}
              className={`text-lg ${index < tour.rating ? "text-yellow-500" : "text-gray-300"
                } mr-1`}
            />
          ))}
        </div>
